...
class PreviewPainter extends CustomPainter {
  ...
  PreviewPainter(Offset leftBottom, {required this.infront, required this.behind}) {
    Path path = Path();

    Offset diff = behind.end.point - infront.start.point;

    path.moveTo(infront.start.point.dx, infront.start.point.dy);
    path.arcToPoint(behind.end.point, radius: Radius.circular(diff.distance / 2), clockwise: false);

    reqSize = Rect.fromPoints(infront.start.point, infront.end.point);
    reqSize = reqSize.expandToInclude(Rect.fromPoints(behind.start.point, behind.end.point));
    ...
    _path.arcToPoint(behind.end.scaled!, radius: Radius.circular(diff.distance / 2), clockwise: false);
  }
