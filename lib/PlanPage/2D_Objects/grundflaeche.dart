...
class Grundflaeche extends Flaeche {
  ...
  void paintGrundflaeche(Canvas canvas) {
    ...
    for (Einkerbung einkerbung in _einkerbungen) {
      areaPath = Path.combine(
          PathOperation.difference, super.areaPath, einkerbung.areaPath);
...
//TODO: TEMP
  ...
  bool containsFullWall(Wall wall, {bool forEinkerbung = false}) {
    ...
    if (forEinkerbung) {
      for (Einkerbung einkerbung in _einkerbungen) {
        temp = Path.combine(
            PathOperation.difference, temp, einkerbung.unscaledPath);
      }
    }
    ...
    pathMetrics.toList().forEach((element) {
      for (var i = 0; i < element.length; i++) {
        ...
        if (tangent != null) {
          Offset pos = Offset(tangent.position.dx.roundToDouble(),
              tangent.position.dy.roundToDouble());
  ...
  Future<double> findMaxLength(Corner startingPoint, double angle,
      {bool forEinkerbung = false}) async {
    ...
    double length = 0;
    Offset einheitsVektor =
        Offset.fromDirection((angle - 90) * pi / 180, stepLengths.first);
