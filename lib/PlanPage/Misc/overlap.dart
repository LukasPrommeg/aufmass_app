...
class Overlap {
  ...
  Overlap({required this.einkerbung, required this.werkstoff}) {
    ...
    if (laibungIntersects.isNotEmpty ||
        laibungOverlaps.isNotEmpty ||
        flaeche != null) {
      _isOverlapping = true;
    }
  }
  ...
  List<Wall> removeDuplicateWalls(List<Wall> walls) {
    ...
    for (int i = 0; i < avaivable.length; i++) {
      List<Wall> matches = avaivable
          .where((element) => wallsAreEqual(avaivable[i], element))
          .toList();
      matches.removeLast();
  ...
  List<Wall> sortWallsAndRemoveDuplicates(List<Wall> walls) {
    ...
    for (int i = 1; i < walls.length; i++) {
      Wall? matchEndStart = avaivable.firstWhereOrNull(
          (element) => doEndsMatch(out.last.end, element.start));
      Wall? matchEndEnd = avaivable.firstWhereOrNull(
          (element) => doEndsMatch(out.last.end, element.end));
      if (matchEndEnd != null) {
        avaivable.remove(matchEndEnd);
        matchEndEnd =
            Wall.fromPoints(start: matchEndEnd.end, end: matchEndEnd.start);
        out.add(matchEndEnd);
...
/*    try {
  ...
  bool doEndsMatch(Corner end1, Corner end2) {
    if (end1.point.dx.roundToDouble() == end2.point.dx.roundToDouble() &&
        end1.point.dy.roundToDouble() == end2.point.dy.roundToDouble()) {
      return true;
    }
    return false;
  }
  ...
  bool wallsAreEqual(Wall wall1, Wall wall2) {
    if (wall1.start.point.dx.roundToDouble() ==
            wall2.start.point.dx.roundToDouble() &&
        wall1.start.point.dy.roundToDouble() ==
            wall2.start.point.dy.roundToDouble() &&
        wall1.end.point.dx.roundToDouble() ==
            wall2.end.point.dx.roundToDouble() &&
        wall1.end.point.dy.roundToDouble() ==
            wall2.end.point.dy.roundToDouble()) {
      return true;
    } else if (wall2.start.point.dx.roundToDouble() ==
            wall1.start.point.dx.roundToDouble() &&
        wall2.start.point.dy.roundToDouble() ==
            wall1.start.point.dy.roundToDouble() &&
        wall2.end.point.dx.roundToDouble() ==
            wall1.end.point.dx.roundToDouble() &&
  ...
  List<Wall> calcLaibungIntersects(List<Wall> lines) {
    ...
    for (Wall borderSide in einkerbung.walls) {
      for (Wall line in lines) {
        Path lineIntersect = Path.combine(PathOperation.intersect,
            line.unscaledPath, borderSide.unscaledPath);
        if (!lineIntersect.getBounds().isEmpty) {
          Offset diff = lineIntersect.getBounds().topLeft -
              lineIntersect.getBounds().bottomRight;
          if (diff.distance.abs() <= 0.1) {
            Offset point = lineIntersect.getBounds().center;
            if (laibungIntersects
                .where((element) =>
                ...
                .isEmpty) {
              Corner laibungIntersect =
                  Corner.fromPoint(point: lineIntersect.getBounds().center);
  ...
  List<Wall> calcWerkstofflinesInsideEinkerbung(List<Wall> lines) {
    List<Wall> result = [];
    for (Wall line in lines) {
      Path overlap = Path.combine(
          PathOperation.intersect, line.unscaledPath, einkerbung.unscaledPath);
  ...
  List<Wall> calcBorderInsideWerkstoffarea() {
    ...
    einkerbung.walls.add(einkerbung.lastWall);
    for (Wall borderSide in einkerbung.walls) {
      Path overlap = Path.combine(PathOperation.intersect,
          borderSide.unscaledPath, werkstoff.clickAble.unscaledPath);
      if (!overlap.getBounds().isEmpty) {
        if (overlap.getBounds().size.longestSide >= 0.1) {
          Wall laibungOverlap = calcLengthOfOverlap(Wall.clone(borderSide));
          laibungOverlap.selected = true;
  ...
  Wall calcLengthOfOverlap(Wall input) {
    ...
    if (limits.length == 1) {
      if (einkerbung.unscaledPath.contains(input.start.point) &&
          werkstoff.clickAble.unscaledPath.contains(input.start.point)) {
        limits.add(Corner.clone(input.start));
      } else if (einkerbung.unscaledPath.contains(input.end.point) &&
          werkstoff.clickAble.unscaledPath.contains(input.end.point)) {
        limits.add(Corner.clone(input.end));
      }
    }
    if (limits.length == 2) {
      return Wall.fromPoints(
          start: Corner.clone(limits.first), end: Corner.clone(limits.last));
